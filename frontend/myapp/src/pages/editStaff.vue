  ...
  try {
    ...
    if (data.schedule && data.schedule.length > 0) {
      data.schedule.forEach((s) => {
        const dayIndex = staffData.value.schedule.findIndex(
          (d) => d.dayOfWeek === dayMapReverse[s.day_of_week],
        );
        if (dayIndex !== -1) {
    ...
    <div class="flex-1 min-h-screen flex items-center justify-center">
      ...
      <form class="flex flex-col gap-4" @submit.prevent="updateStaff">
        ...
        <div class="border-t border-slate-200 pt-6 mt-4">
          ...
          <div
            v-for="day in staffData.schedule"
            :key="day.dayOfWeek"
            class="mb-4 p-4 border border-slate-200 rounded-lg"
          >
